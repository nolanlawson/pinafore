...
test('uploads alts for media', async t => {
  ...
  await t.typeText(getNthMediaAltInput(2), 'kitten 2')
    ...
    .click(composeButton)
    .expect(getNthStatusAndImage(1 + 0, 0).getAttribute('alt')).eql('kitten 1')
    .expect(getNthStatusAndImage(1 + 0, 0).getAttribute('title')).eql('kitten 1')
    .expect(getNthStatusAndImage(1 + 0, 1).getAttribute('alt')).eql('kitten 2')
    .expect(getNthStatusAndImage(1 + 0, 1).getAttribute('title')).eql('kitten 2')
})
...
test('uploads alts when deleting and re-uploading media', async t => {
  ...
  await t.expect(getNthMediaAltInput(1).value).eql('')
    ...
    .click(composeButton)
    .expect(getNthStatusAndImage(1 + 0, 0).getAttribute('alt')).eql('this will not be deleted')
})
...
test('uploads alts mixed with no-alts', async t => {
  ...
  await t.typeText(getNthMediaAltInput(2), 'kitten numero dos')
    .click(composeButton)
    .expect(getNthStatusAndImage(1 + 0, 0).getAttribute('alt')).eql('')
    .expect(getNthStatusAndImage(1 + 0, 1).getAttribute('alt')).eql('kitten numero dos')
})
...
test('saves alts to local storage', async t => {
  ...
  await t.typeText(getNthMediaAltInput(1), 'kitten numero uno')
    ...
    .click(composeButton)
    .expect(getNthStatusAndImage(1 + 0, 0).getAttribute('alt')).eql('kitten numero uno')
    .expect(getNthStatusAndImage(1 + 0, 1).getAttribute('alt')).eql('kitten numero dos')
})
...
test('can post a status with empty content if there is media', async t => {
  ...
  await t.click(composeButton)
    .expect(getNthStatusAndImage(1 + 0, 0).getAttribute('alt')).eql('just an image!')
})
