...
test('deleted statuses are removed from the timeline', async t => {
  ...
  await t.expect(getNthStatus(1 + 0).innerText).contains("I'm gonna delete this", { timeout })
  await deleteAs('admin', status.id)
  await t.expect(getNthStatus(1 + 0).innerText).notContains("I'm gonna delete this", { timeout })
  ...
  await t
    .navigateTo('/')
    .expect(getNthStatus(1 + 0).innerText).notContains("I'm gonna delete this", { timeout })
})
...
test('deleted statuses are removed from threads', async t => {
  ...
  await t.expect(getNthStatus(1 + 0).innerText).contains('But I will delete this', { timeout })
    .expect(getNthStatus(1 + 1).innerText).contains("I won't delete this", { timeout })
    .click(getNthStatus(1 + 1))
    .expect(getUrl()).contains('/statuses')
    .expect(getNthStatus(1 + 0).innerText).contains("I won't delete this", { timeout })
    .expect(getNthStatus(1 + 1).innerText).contains('But I will delete this', { timeout })
  await deleteAs('admin', reply.id)
  await t.expect(getNthStatus(1 + 1).exists).notOk()
    .expect(getNthStatus(1 + 0).innerText).contains("I won't delete this", { timeout })
  await t.navigateTo('/')
  await forceOffline()
  await t.click(getNthStatus(1 + 0))
    .expect(getUrl()).contains('/statuses')
    .expect(getNthStatus(1 + 1).exists).notOk()
    .expect(getNthStatus(1 + 0).innerText).contains("I won't delete this", { timeout })
  await forceOnline()
})
