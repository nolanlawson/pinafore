...
test('Can pin statuses', async t => {
  ...
  await t
    ...
    .expect(getNthStatus(1 + 0).innerText).contains('I am going to pin this')
    ...
    .expect(getNthPinnedStatus(1 + 0).innerText).contains('I am going to pin this')
    .expect(getNthPinnedStatus(1 + 1).innerText).contains('this is unlisted')
    .expect(getNthStatus(1 + 0).innerText).contains('I am going to pin this')
    .click(getNthStatusOptionsButton(1 + 0))
    .expect(getNthDialogOptionsOption(1).innerText).contains('Delete')
...
test('Can favorite a pinned status', async t => {
  ...
  await t
    .click(avatarInComposeBox)
    .expect(getNthPinnedStatus(1 + 0).getAttribute('aria-setsize')).eql('1')
    .expect(getNthPinnedStatusFavoriteButton(1 + 0).getAttribute('aria-pressed')).eql('false')
    .click(getNthPinnedStatusFavoriteButton(1 + 0))
    .expect(getNthPinnedStatusFavoriteButton(1 + 0).getAttribute('aria-pressed')).eql('true')
    .click(getNthPinnedStatusFavoriteButton(1 + 0))
    .expect(getNthPinnedStatusFavoriteButton(1 + 0).getAttribute('aria-pressed')).eql('false')
})
...
test('Saved pinned/unpinned state of status', async t => {
  ...
  await loginAsFoobar(t)
  await t
    .expect(getNthStatusContent(1 + 0).innerText).contains('hey I am going to pin and unpin this', { timeout })
    .click(getNthStatusOptionsButton(1 + 0))
    .expect(getNthDialogOptionsOption(2).innerText).contains('Pin to profile')
    .click(getNthDialogOptionsOption(2))
  await sleep(1)
