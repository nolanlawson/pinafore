...
test('shows unread notification', async t => {
  ...
  await t
    ...
    .expect(getNthStatusContent(1 + 0).innerText).contains('somebody please favorite this to validate me', {
      timeout: 20000
    })
  ...
  await t
    ...
    .expect(getNthStatus(1 + 0).innerText).contains('somebody please favorite this to validate me')
    .expect(getNthStatus(1 + 0).innerText).match(/admin\s+favorited your status/)
  await t
    .click(homeNavButton)
    .expect(notificationsNavButton.getAttribute('aria-label')).eql('Notifications')
    .expect(getTitleText()).eql('localhost:3000 · Home')
...
test('shows unread notifications, more than one', async t => {
  ...
  await t
    .expect(notificationsNavButton.getAttribute('aria-label')).eql('Notifications')
    .expect(getTitleText()).eql('localhost:3000 · Home')
    .expect(getNthStatusContent(1 + 0).innerText).contains('I need lots of favorites on this one', {
      timeout: 20000
    })
  await favoriteStatusAs('admin', id)
  ...
  await t
    ...
    .expect(notificationsNavButton.getAttribute('aria-label')).eql('Notifications (current page)')
    .expect(getTitleText()).eql('localhost:3000 · Notifications')
    .expect(getNthStatus(1 + 0).innerText).contains('I need lots of favorites on this one')
  await t
    .click(homeNavButton)
    .expect(notificationsNavButton.getAttribute('aria-label')).eql('Notifications')
    .expect(getTitleText()).eql('localhost:3000 · Home')
