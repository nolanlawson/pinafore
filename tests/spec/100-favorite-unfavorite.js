...
test('favorites a status', async t => {
  ...
  await t
    ...
    .expect(getNthFavorited(1 + 4)).eql('false')
    .click(getNthFavoriteButton(1 + 4))
    .expect(getNthFavorited(1 + 4)).eql('true')
...
test('unfavorites a status', async t => {
  await loginAsFoobar(t)
  await t
    .expect(getNthFavorited(1 + 1)).eql('true')
    .click(getNthFavoriteButton(1 + 1))
    .expect(getNthFavorited(1 + 1)).eql('false')
...
test('Keeps the correct favorites count', async t => {
  ...
  await t
    ...
    .expect(getNthFavorited(1 + 0)).eql('true')
    ...
    .click(getNthStatus(1 + idx))
    .expect(getUrl()).contains('/status')
    .expect(getNthFavorited(1 + 0)).eql('false')
