...
test('Fav stats update', async t => {
  ...
  await t
    .expect(getNthStatusContent(1 + 0).innerText).contains('hey hello look at this toot')
...
test('Reblog stats update', async t => {
  ...
  await loginAsFoobar(t)
  await t
    .expect(getNthStatusContent(1 + 0).innerText).contains('oh why hello it looks like another toot')
    .click(getNthStatus(1 + 0))
...
test('Fav and reblog stats update for a boosted toot', async t => {
  ...
  await loginAsFoobar(t)
  await t
    .expect(getNthStatusContent(1 + 0).innerText).contains('this will get boosted')
    .click(getNthStatus(1 + 0))
    .expect(getReblogsCount()).eql(1)
    .expect(getFavoritesCount()).eql(2)
    .click(homeNavButton)
