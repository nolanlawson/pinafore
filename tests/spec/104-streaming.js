...
test('new incoming statuses show up immediately', async t => {
  ...
  await t.expect(getNthStatus(1 + 0).innerText).contains('hello my baby hello my honey')
})
...
test('new incoming toots show a button if scrolled down', async t => {
  ...
  await sleep(4000)
  await t.hover(getNthStatus(1 + 2))
    .hover(getNthStatus(1 + 0))
  await scrollToTop()
  await sleep(1000)
  await t
  ...
  await t
    .expect(getNthStatus(1 + 0).innerText).contains("baby my heart's on fire")
    .expect(getNthStatus(1 + 1).innerText).contains('send me a kiss by wire')
    .expect(getNthStatus(1 + 2).innerText).contains('hello my ragtime gal')
    .navigateTo('/')
    .expect(getNthStatus(1 + 0).innerText).contains("baby my heart's on fire")
    .expect(getNthStatus(1 + 1).innerText).contains('send me a kiss by wire')
    .expect(getNthStatus(1 + 2).innerText).contains('hello my ragtime gal')
