...
test('replies have same privacy as replied-to status by default', async t => {
  ...
  await t
    ...
    .click(getNthReplyButton(1 + 1))
    .expect(getNthPostPrivacyButton(1 + 1).getAttribute('aria-label')).eql('Adjust privacy (currently Unlisted)')
    .click(getNthReplyButton(1 + 1))
    .hover(getNthStatus(1 + 2))
    .click(getNthReplyButton(1 + 2))
    .expect(getNthPostPrivacyButton(1 + 2).getAttribute('aria-label')).eql('Adjust privacy (currently Followers-only)')
    .click(getNthReplyButton(1 + 2))
    .hover(getNthStatus(1 + 3))
    .hover(getNthStatus(1 + 4))
    .hover(getNthStatus(1 + 5))
    .click(getNthReplyButton(1 + 5))
    .expect(getNthPostPrivacyButton(1 + 5).getAttribute('aria-label')).eql('Adjust privacy (currently Public)')
    .click(getNthReplyButton(1 + 5))
})
...
test('replies save changes to post privacy', async t => {
  ...
  await t
    ...
    .expect(getNthPostPrivacyButton(1 + 1).getAttribute('aria-label')).eql('Adjust privacy (currently Public)')
    .click(getNthStatusRelativeDate(1 + 1))
    .expect(getUrl()).contains('/statuses')
    .click(getNthReplyButton(1 + 0))
    .expect(getNthPostPrivacyButton(1 + 0).getAttribute('aria-label')).eql('Adjust privacy (currently Public)')
})
