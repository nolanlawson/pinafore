...
test('Shortcut j/k change the active status', async t => {
  ...
  await t
    ...
    .expect(getNthStatus(1 + 0).exists).ok({ timeout: 30000 })
    .expect(isNthStatusActive(1 + 0)()).notOk()
    .pressKey('j')
    .expect(isNthStatusActive(1 + 0)()).ok()
    .pressKey('j')
    .expect(isNthStatusActive(1 + 1)()).ok()
    .pressKey('j')
    .expect(isNthStatusActive(1 + 2)()).ok()
    .pressKey('j')
    .expect(isNthStatusActive(1 + 3)()).ok()
    .pressKey('k')
    .expect(isNthStatusActive(1 + 2)()).ok()
    .pressKey('k')
    .expect(isNthStatusActive(1 + 1)()).ok()
    .pressKey('k')
    .expect(isNthStatusActive(1 + 0)()).ok()
    .expect(isNthStatusActive(1 + 1)()).notOk()
    .expect(isNthStatusActive(1 + 2)()).notOk()
    .expect(isNthStatusActive(1 + 3)()).notOk()
})
...
test('Shortcut j goes to the first visible status', async t => {
  ...
  await t
    .expect(getUrl()).eql('http://localhost:4002/')
  await scrollToStatus(t, 1 + 10)
  await t
    .expect(getNthStatus(1 + 10).exists).ok({ timeout: 30000 })
    .pressKey('j')
...
test('Shortcut o opens active status, backspace goes back', async t => {
  ...
  await t
    .expect(getUrl()).eql('http://localhost:4002/')
    .expect(getNthStatus(1 + 2).exists).ok({ timeout: 30000 })
