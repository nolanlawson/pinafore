...
test('reply to non-focused parent status in a thread', async t => {
  ...
  await t
    ...
    .expect(getNthStatusContent(1 + 0).innerText).contains('and here it is continued')
...
test('reply to focused status in a thread', async t => {
  ...
  await t
    .hover(getNthStatus(1 + 0))
    .expect(getNthStatusContent(1 + 0).innerText).contains('whoa and here it is probably continued')
    .hover(getNthStatusContent(1 + 1))
...
test('reply to non-focused grandparent status in a thread', async t => {
  ...
  await t
    .hover(getNthStatus(1 + 0))
    .expect(getNthStatusContent(1 + 0).innerText).contains('cool thread 3/3')
    .click(getNthStatus(1 + 0))
    .expect(getUrl()).match(/statuses/)
...
test('reply to non-focused grandchild status in a thread', async t => {
  ...
  await t
    .hover(getNthStatus(1 + 0))
    .expect(getNthStatusContent(1 + 0).innerText).contains('sweet thread 3/3')
    .hover(getNthStatusContent(1 + 1))
    .hover(getNthStatus(1 + 2))
