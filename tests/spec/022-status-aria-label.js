...
test('basic aria-labels for statuses', async t => {
  ...
  await t
    ...
    .hover(getNthStatus(1 + 0))
    .expect(getNthStatus(1 + 1).getAttribute('aria-label')).match(
...
test('aria-labels for notifications', async t => {
  ...
  await t
    ...
    .expect(getNthStatus(1 + 2).getAttribute('aria-label')).match(
      /admin, @foobar notification of unlisted message, .* ago, @admin, Unlisted/i
    )
  await scrollToStatus(t, 1 + 4)
  await t
    .hover(getNthStatus(1 + 4))
    .expect(getNthStatus(1 + 4).getAttribute('aria-label')).match(
      /admin, @foobar notification of direct message, .* ago, @admin, Direct/i
    )
  await scrollToStatus(t, 1 + 5)
  await t
    .hover(getNthStatus(1 + 5))
    .expect(getNthStatus(1 + 5).getAttribute('aria-label')).match(
